...
8. **Under the hood tab** (0014, 0003)
   ...
   - ✅ **Done 2026-09-15.** Typecheck, lint, build and Prettier pass; Vitest **104 pass (9 new)**. `dotnet` untouched. Checked in headless Chrome against the live API: GQ-03 on Stage 5 (Constrain) and Hybrid (RRF), GQ-02 on Keyword (no matches), GQ-01 on Structured (SQL). Built:
     - `UnderTheHoodTab`: `TraceFlow` (a Radix tablist of step chips, coloured by each step's own stage, ←/→ between them) and `TraceStepView` (title, stage, duration, the step's view, its SQL and parameters, its notes). It opened on the last step, where the stage's own technique runs; **changed 2026-09-18** to open on the first, so the trace is read forwards as the story of how the results were made.
   ...
   - Findings:
     ...
     - The selected step isn't in the URL (local state), so a bookmark opens on the first step. Talk mode (step 10) can add it if a talk step needs to open on, say, RRF.
...
8. **Clean-up and sign-off (Pete, once the whole demo is complete)** (0014):
   ...
   - **Stage 6–7 content drafted in Phase 4:** `content/stages/rag.md` and `pedagogy.md`; talk steps `stage-rag` and `stage-pedagogy` and their captions (the three Stage 7 steps became one in step 4a); the AI-stage glossary entries; UI labels in the Answer tab and the Stage 6–7 trace views.
   - **Going further content drafted in step 4a:** `content/going-further/*.md` (six files) and the new glossary entries, plus the rewritten closing `content/talk/going-further.md`.
